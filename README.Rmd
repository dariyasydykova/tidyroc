...
Before I plot any curves, I fit two logitistic regression models. I will use the `iris` dataset to fit the models to. First, I need to reduce `iris` to only contain data for two species of irises. This ensures that our outcome is binary. 
...
I reduce the `iris` dataset to contain information only about virginica and versicolor. The first model I fit tries to predict species with predictors petal width, petal length, and sepal width. The second I fit tries to predict species with predictor sepal width.
